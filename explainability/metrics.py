...
class XAIMetrics:
    ...
    def faithfulness_insertion(self, x, attribution, steps=10, target=None):
        ...
        with torch.no_grad():
            ...
            if target is None:
                target = orig_output.argmax(dim=1)
        ...
        B = x.shape[0]
        x_flat = x.view(B, -1)
        attr_flat = attribution.abs().view(B, -1)
        ...
        for step in range(steps + 1):
            ...
            for b in range(B):
                ...
                x_masked.view(B, -1)[b, indices] = x_flat[b, indices]
    ...
    def robustness_stability(self, x, explainer, n_perturbations=10, noise_std=0.1):
        ...
        base_attr_flat = base_attr.view(x.shape[0], -1).cpu().numpy()
        ...
        for _ in range(n_perturbations):
            ...
            perturbed_attr_flat = perturbed_attr.view(x.shape[0], -1).cpu().numpy()
