...
As shown by the negative rank correlation ($-0.09$ on CLE4EVR, $-0.24$ on MNMath) in Fig. \ref{fig:heatmap}, SHAP and the internal reasoning of ProtoPNet are in complete disagreement. 
\begin{itemize}
    \item \textbf{SHAP} highlights fine edges and bright pixels, producing a highly sparse explanation (Complexity: 0.001).
    \item \textbf{ProtoPNet Internal} highlights broad, diffuse spatial regions, including the objects and their surrounding context (Complexity: 0.52, see Fig. \ref{fig:grid_comparison}).
\end{itemize}
